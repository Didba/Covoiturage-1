	...
	class CarburantManager
	{
		...
		**/
		function add(array $data){
			extract($data);
			$query = $this->_db->prepare('INSERT INTO covoiturage_carburant(libelle) VALUES (:libelle)');
		...
		**/
		function remove(array $data){
			...
			{
				$query = $this->_db->prepare('DELETE FROM covoiturage_carburant WHERE id_carburant=:id_carburant');
				$query -> bindParam(':id_carburant', $id_carburant,PDO::PARAM_INT);
			}
			else if(isset($libelle))
			{
				$query = $this->_db->prepare('DELETE FROM covoiturage_carburant WHERE libelle=:libelle');
				$query -> bindParam(':libelle', $libelle,PDO::PARAM_STR);
			}
			$query->execute() or die(print_r($query->errorInfo()));
		}
		...
		**/
		function get(array $data){
			...
			if(isset($id_carburant))
			{
				$query = $this->_db->prepare('SELECT * FROM covoiturage_carburant WHERE id_carburant=:id_carburant');
				$query -> bindParam(':id_carburant', $id_carburant,PDO::PARAM_INT);
				$query->execute() or die(print_r($query->errorInfo()));
				$result = $query->fetch();
			}
			...
			{
				$query = $this->_db->prepare('SELECT * FROM covoiturage_carburant WHERE libelle=:libelle');
				$query -> bindParam(':libelle', $libelle,PDO::PARAM_STR);
				$query->execute() or die(print_r($query->errorInfo()));
				$result = $query->fetch();
			}
		...
		**/
		function getList($champs=NULL){
			...
			if($champs==NULL)
			{
				$query = $this->_db->prepare('SELECT * FROM covoiturage_carburant');
			}
			else
			{
				$query_str = "SELECT * FROM covoiturage_carburant WHERE 1"; //Début de la requête. Le WHERE 1 (toujours vrai) est là pour faciliter la boucle qui suit et que le "statement" puisse toujours commencer par " AND" m^me s'il s'agit du premier champ
		...
		**/
		function update($carburant){
			extract($carburant);
			$query = $this->_db->prepare('UPDATE covoiturage_carburant SET libelle=:libelle WHERE id_carburant=:id_carburant');
			$query -> bindParam(':id_carburant', $id_carburant,PDO::PARAM_INT);
			$query -> bindParam(':libelle', $libelle,PDO::PARAM_STR);
			$query->execute() or die(print_r($query->errorInfo()));
		}
