	...
	class caracteristiquesManager
	{
		...
		**/
		function remove(array $data){
			...
			if(isset($id))
			{
				$query = $this->_db->prepare('DELETE FROM caracteristique WHERE id=:id');
		...
		/**
		* Fonction permettant de récupérer un caracteristique. Paramètre: array contenant soit id=>$id soit nom=>$nom (pour que la recherche fonctionne avec l'un et l'autre)
		**/
		function get(array $data){
			extract($data);
			if(isset($id))
			{
				$query = $this->_db->prepare('SELECT * FROM caracteristique WHERE id=:id');
				$query -> bindParam(':id', $id,PDO::PARAM_INT);
			}
			else if(isset($nom))
			{
			...
			$result = $query->fetch();
			$result['caracteristique'] = $this->_CarManager->get(array("id"=>$result['caracteristique']));
		...
		**/
		function getList($champs=NULL){
			...
			foreach ($result as $key => &$value) {
				$caracteristique = new caracteristique();
				$value['caracteristique'] = $this->_CarManager->get(array("id"=>$value['caracteristique']));
		...
		**/
		function update($caracteristique){
			...
			$query = $this->_db->prepare('UPDATE caracteristique SET nom=:nom WHERE id=:id');
			$query -> bindParam(':id', $id,PDO::PARAM_INT);
			$query -> bindParam(':nom', $nom,PDO::PARAM_STR);
			$query->execute() or die(print_r($query->errorInfo()));
		}
